...
Flight day reminder emails sent via Resend with React Email templates. A stateless Vercel cron queries `SharedSchedule` directly every 15 minutes — no separate scheduling table. `flightDayEmailSentAt` on `SharedSchedule` prevents duplicate sends.

See `email-design.md` for full architecture, send time calculation, template structure, security, and configuration.
